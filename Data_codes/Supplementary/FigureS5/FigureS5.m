%% FIGURE S5: Reaction Time (2(task instructions:Recall, Suppress) x 3 (emotions: Negative, Neutral, Positive))
% -------------------------------------------------------------------------
% This script reproduces Figure S5 from the manuscript and completes:
...
sample_size = length(NEG_Thk_correct_RT);
%% --------------------------- DEFINE COLORS ---------------------------------------
...
NEG_Thk_correct_RT = zscore(NEG_Thk_correct_RT);
NU_Thk_correct_RT = zscore(NU_Thk_correct_RT);
POS_Thk_CORRECT_RT = zscore(POS_Thk_CORRECT_RT);

NEG_NT_correct_RT = zscore(NEG_NT_correct_RT);
NU_NT_correct_RT = zscore(NU_NT_correct_RT);
POS_NT_CORRECT_RT = zscore(POS_NT_CORRECT_RT);

Ymat = [NEG_Thk_correct_RT(:), NEG_NT_correct_RT(:), ...
        NU_Thk_correct_RT(:),  NU_NT_correct_RT(:), ...
        POS_Thk_CORRECT_RT(:), POS_NT_CORRECT_RT(:)];
...
%% ---------------------- SCATTER: RECALL ----------------------------------
X = [x_NEG, x_NU, x_POS, x_NEG_NT, x_NU_NT, x_POS_NT];

Y = {NEG_Thk_correct_RT, NU_Thk_correct_RT, POS_Thk_CORRECT_RT, ...
     NEG_NT_correct_RT,  NU_NT_correct_RT,  POS_NT_CORRECT_RT};

Ymean = {NEG_Thk_correct_RT, NU_Thk_correct_RT, POS_Thk_CORRECT_RT, ...
         NEG_NT_correct_RT,  NU_NT_correct_RT,  POS_NT_CORRECT_RT};
...
ylabel('Reaction Time (z-scores)','FontSize',14);
...
data = {
    NEG_Thk_correct_RT, 'Negative Recall';
    POS_Thk_CORRECT_RT, 'Positive Recall';
    NU_Thk_correct_RT,  'Neutral Recall';
    NEG_NT_correct_RT,'Negative Suppress';
    POS_NT_CORRECT_RT,'Positive Suppress';
    NU_NT_correct_RT, 'Neutral Suppress'
};
...
[p, tbl, stats] = friedman(Ymat)

%% ===================== SAVE FIGURE (300 DPI) ============================

% Save as PNG (300 dpi)
exportgraphics(gca,'FigureS5.png','Resolution',300);

% Save as TIFF (300 dpi)
exportgraphics(gca,'FigureS5.tif','Resolution',300);