%% FIGURE S2: Correlation between Balance Integration Scores and Sensitivity
% -------------------------------------------------------------------------
...
%   - Data file: 'FigureS2.mat' in present working directory 
%
%
% OUTPUT:
...
[b_negT,stats_negT] = robustfit(negT', zscore_dprime_negT');
[b_posT,stats_posT] = robustfit(posT', zscore_dprime_posT');
[b_nuT,stats_nuT] =robustfit(nuT', zscore_dprime_nuT');

[b_negNT,stats_negNT] = robustfit(negNT', zscore_dprime_negNT');
[b_posNT,stats_posNT] = robustfit(posNT', zscore_dprime_posNT');
[b_nuNT,stats_nuNT] = robustfit(nuNT', zscore_dprime_nuNT');
...
Y = {zscore_dprime_negT, zscore_dprime_negNT, zscore_dprime_nuT, zscore_dprime_nuNT, zscore_dprime_posT, zscore_dprime_posNT};
...
r_robust_negT = corr( zscore_dprime_negT',y_pred_negT);% coefficient of robust regression

y_pred_negNT = b_negNT(1) + b_negNT(2)* negNT';
r_robust_negNT = corr( zscore_dprime_negNT',y_pred_negNT);% coefficient of robust regression


y_pred_nuT = b_nuT(1) + b_nuT(2)*nuT';
r_robust_nuT = corr(  zscore_dprime_nuT',y_pred_nuT);% coefficient of robust regression


y_pred_nuNT = b_nuNT(1) + b_nuNT(2)*nuNT';
r_robust_nuNT = corr(  zscore_dprime_nuNT',y_pred_nuNT);% coefficient of robust regression

y_pred_posT = b_posT(1) + b_posT(2)*posT';
r_robust_posT = corr(  zscore_dprime_posT', y_pred_posT);% coefficient of robust regression

y_pred_posNT = b_posNT(1) + b_posNT(2)*posNT';
r_robust_posNT = corr(  zscore_dprime_posNT', y_pred_posNT);% coefficient of robust regression
